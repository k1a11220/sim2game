...
export async function loadSceneFromURL(mujoco, filename, parent) {
    ...
    try {
      ...
      console.error(`Failed to load MuJoCo XML: ${filename}`, err, {
        name: err?.name,
        message: err?.message,
        stack: err?.stack,
      });
      if (typeof mujoco.getExceptionMessage === 'function') {
        try {
          console.error('MuJoCo exception:', mujoco.getExceptionMessage(err));
        } catch (nested) {
          console.warn('Unable to decode MuJoCo exception message', nested);
        }
      }
      throw err;
    }
...
export async function downloadExampleScenesFolder(mujoco) {
  let allFiles = DEFAULT_SCENE_FILES;
  try {
    const indexResponse = await fetch("./examples/scenes/index.json");
    if (indexResponse.ok) {
      const filesFromIndex = await indexResponse.json();
      if (Array.isArray(filesFromIndex) && filesFromIndex.length > 0) {
        allFiles = filesFromIndex;
      } else {
        console.warn("examples/scenes/index.json is empty; using default scene list");
      }
    } else {
      console.warn("Failed to load examples/scenes/index.json (", indexResponse.status, ") using default scene list");
    }
  } catch (err) {
    console.warn("Could not load examples/scenes/index.json; using default scene list", err);
  }
