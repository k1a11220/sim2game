...
function safeAnalyze(module: MujocoModule, target: string) {
  try {
...
function ensureDirRecursive(module: MujocoModule, dirPath: string) {
  if (!dirPath || dirPath === '/') {
    return;
  }
...
export function ensureWorkingDirectory(module: MujocoModule, path = DEFAULT_WORKING_DIR) {
...
async function resolveData(descriptor: PreloadFileDescriptor): Promise<string | Uint8Array> {
...
function ensureParentDir(module: MujocoModule, filePath: string) {
...
export async function preloadFiles(
  module: MujocoModule,
  descriptors: PreloadFileDescriptor[] = [],
  baseDir: string = DEFAULT_WORKING_DIR,
) {
