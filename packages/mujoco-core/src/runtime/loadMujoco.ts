...
import type { CreateOptions, MujocoModule } from '../types.js';
...
export async function loadMujocoModule(options: CreateOptions = {}): Promise<MujocoModule> {
  ...
  const overrides: Record<string, unknown> = { ...moduleOverrides };
  ...
  const module = (await loadMujocoFactory(overrides)) as MujocoModule;
  ...
  if ('ready' in module && module.ready instanceof Promise) {
    await awaitWithTimeout(module.ready as Promise<MujocoModule>, readyTimeoutMs);
  }
